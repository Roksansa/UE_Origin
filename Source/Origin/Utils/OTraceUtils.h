...
namespace OTraceUtils
{
	...
	bool SweepBoxSingleByChannel(const UWorld* InWorld, struct FHitResult& OutHit, const FVector& Start, const FVector& End, FVector& BoxHalfExtent, const FQuat& Rot, ECollisionChannel TraceChannel, const FCollisionQueryParams& Params = FCollisionQueryParams::DefaultQueryParam, const FCollisionResponseParams& ResponseParam = FCollisionResponseParams::DefaultResponseParam, bool bDrawDebug = false, float DrawTime = -1.f, FColor TraceColor = FColor::Black, FColor HitColor = FColor::Red);
	...
	bool OverlapCapsuleBlockingByProfile(const UWorld* InWorld, const FVector& Pos, float CapsuleRadius, float CapsuleHalfHeight, const FQuat& Rot, FName ProfileName, const FCollisionQueryParams& Params = FCollisionQueryParams::DefaultQueryParam, bool bDrawDebug = false, float DrawTime = -1.f, FColor TraceColor = FColor::Green, FColor HitColor = FColor::Yellow);
}